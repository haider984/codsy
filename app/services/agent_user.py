...
from app.models.agent_user import AgentUserCreate, AgentUserUpdate, AgentUserInDB, UserStatus,AgentUserGroqApiUpdate
...
async def create_agent_user(agent_user_data: AgentUserCreate, db: AsyncIOMotorDatabase) -> AgentUserInDB:
    # If the new user is being allowed, set all existing allowed users to not_allowed
    if agent_user_data.status == UserStatus.ALLOWED:
        await db[COLLECTION_NAME].update_many(
            {"status": UserStatus.ALLOWED},
            {"$set": {"status": UserStatus.NOT_ALLOWED}}
        )
...
async def update_agent_user(agent_user_id: str, agent_user_update_data: AgentUserUpdate, db: AsyncIOMotorDatabase) -> Optional[AgentUserInDB]:
    ...
    # If status is being set to allowed, set all others to not_allowed
    if update_data.get("status") == UserStatus.ALLOWED:
        await db[COLLECTION_NAME].update_many(
            {"_id": {"$ne": ObjectId(agent_user_id)}, "status": UserStatus.ALLOWED},
            {"$set": {"status": UserStatus.NOT_ALLOWED}}
        )
...
async def update_agent_user_groq_api(
    agent_user_id: str,
    groq_api_update_data: AgentUserGroqApiUpdate,
    db: AsyncIOMotorDatabase
) -> Optional[AgentUserInDB]:
    if not ObjectId.is_valid(agent_user_id):
        return None

    update_data = groq_api_update_data.model_dump(exclude_unset=True)

    if not update_data:
        # No fields to update; return existing user data
        return await get_agent_user_by_id(agent_user_id, db)

    # Only update Groq API fields, no other logic like status changes
    await db[COLLECTION_NAME].update_one(
        {"_id": ObjectId(agent_user_id)},
        {"$set": update_data}
    )
